...
import { getToken, setToken, hasToken } from '../lib/githubStorage'
...
export default function ManagePortfolio({ holdings, onSave, saving, saveError }) {
  ...
  const addRow = () => {
    ...
    setAddError('')
    if (!ticker) { setAddError('Enter a ticker or ISIN'); return }
    if (rows.some(r => r.ticker === ticker)) { setAddError(`${ticker} already exists`); return }
    ...
    setNewTicker(''); setNewShares(''); setNewCost(''); setNewBroker('')
    setSaved(false)
  ...
  const handleBulkImport = () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!ticker) continue
      if (existingTickers.has(ticker)) { errors.push(`Line ${i + 1}: ${ticker} already exists, skipped`); continue }
  ...
  const handleExport = () => {
    const config = {
      holdings: rows.filter(r => r.ticker).map(({ ticker, shares, avgCost, currency, broker }) => ({
        ticker, shares: Number(shares) || 0, avgCost: Number(avgCost) || 0,
        currency: currency || 'USD', broker: broker || '',
      })),
    }
  ...
  const handleKeyDown = (e) => { if (e.key === 'Enter') addRow() }
  ...
  return (
    <div className="space-y-4">
      {/* GitHub Token Setup */}
      {showToken && (
        <div className="bg-gray-900 rounded-lg border border-bb-amber/30 p-4 space-y-3">
          <div className="text-sm font-medium text-bb-amber">GitHub Token Required</div>
          <div className="text-xs text-gray-400">
            Your portfolio is stored in the Git repo. To save changes, create a
            <a href="https://github.com/settings/personal-access-tokens/new" target="_blank" rel="noopener noreferrer"
              className="text-bb-amber underline ml-1">fine-grained token</a> with
            <strong className="text-gray-300"> Contents: Read &amp; Write</strong> permission
            on <code className="text-gray-300">frutixu/bloomberg-lite</code>.
          </div>
          <div className="flex gap-2">
            <input
              ...
              className={`${inputBase} flex-1 border-gray-600`}
            />
            <button
              onClick={handleSaveToken}
              disabled={!tokenInput.trim()}
              className="px-4 py-1 text-sm bg-bb-amber text-black rounded font-medium hover:bg-bb-amber/90 transition-colors disabled:opacity-40"
            >
              Save Token
            </button>
          </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2 items-center">
          <button onClick={() => setShowToken(!showToken)}
            className={`px-2 py-2 text-xs rounded transition-colors border ${hasToken() ? 'bg-gray-800 text-bb-green border-gray-700' : 'bg-gray-800 text-bb-red border-gray-700'}`}
            title={hasToken() ? 'GitHub token configured' : 'No GitHub token'}>
            {hasToken() ? '● Connected' : '○ No token'}
          </button>
          <button onClick={() => setShowBulk(!showBulk)}
            className={`px-3 py-2 text-sm rounded transition-colors border ${showBulk ? 'bg-bb-amber/10 text-bb-amber border-bb-amber/30' : 'bg-gray-800 text-gray-300 border-gray-700 hover:bg-gray-700'}`}>
            Bulk Import
          </button>
          ...
          <button onClick={handleSave} disabled={saving}
            className={`px-4 py-2 text-sm rounded font-medium transition-all ${
              saving ? 'bg-bb-amber/50 text-black cursor-wait'
      ...
      {saveError && (
        <div className="text-xs text-bb-red bg-bb-red/10 border border-bb-red/20 rounded px-3 py-2">{saveError}</div>
      )}
      ...
      {showBulk && (
        <div className="bg-gray-900 rounded-lg border border-bb-amber/20 p-4 space-y-3">
          <div className="text-xs text-gray-400">
            ...
            <br />Only ticker is required. You can use tickers, ISINs, or product names.
          </div>
...
AAPL`}
          ...
          <textarea value={bulkText} onChange={e => setBulkText(e.target.value)}
            placeholder="Paste rows here..." rows={6}
      ...
      <div className="bg-gray-900 rounded-lg border border-gray-800 overflow-x-auto">
            ...
            {rows.map((row, i) => (
              <tr key={row.key} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                <td className="px-3 py-2">
                  <input type="text" value={row.ticker} onChange={e => updateRow(i, 'ticker', e.target.value)}
                    className={`${inputBase} w-36 font-bold`} />
                </td>
                <td className="px-3 py-2">
                  <input type="text" value={row.broker || ''} onChange={e => updateRow(i, 'broker', e.target.value)}
                    placeholder="--" className={`${inputBase} w-20 placeholder-gray-600`} />
                </td>
                <td className="px-3 py-2">
                  <select value={row.currency || 'USD'} onChange={e => updateRow(i, 'currency', e.target.value)} className={`${inputBase} w-20`}>
                ...
                <td className="px-3 py-2">
                  <input type="number" value={row.shares} onChange={e => updateRow(i, 'shares', e.target.value)}
                    min="0" step="any" className={`${inputBase} w-20`} />
                </td>
                <td className="px-3 py-2">
                  <div className="flex items-center gap-1">
                    <span className="text-gray-500 text-sm">{sym(row.currency)}</span>
                    <input type="number" value={row.avgCost} onChange={e => updateRow(i, 'avgCost', e.target.value)}
                      min="0" step="0.01" className={`${inputBase} w-24`} />
                  </div>
                </td>
                ...
                <td className="px-1 py-2 text-center">
                  <button onClick={() => removeRow(i)} className="text-gray-600 hover:text-red-400 transition-colors text-sm px-1" title="Remove">×</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="border-t border-gray-700 bg-gray-800/30 px-3 py-3 flex items-center gap-2 flex-wrap">
          <input ref={tickerRef} type="text" value={newTicker}
            onChange={e => { setNewTicker(e.target.value); setAddError('') }}
            onKeyDown={handleKeyDown} placeholder="TICKER / ISIN"
            className={`${inputBase} w-36 placeholder-gray-600 font-bold border-gray-600`} />
          <input type="text" value={newBroker} onChange={e => setNewBroker(e.target.value)}
            onKeyDown={handleKeyDown} placeholder="Broker"
            className={`${inputBase} w-20 placeholder-gray-600 border-gray-600`} />
          <select value={newCurrency} onChange={e => setNewCurrency(e.target.value)} className={`${inputBase} w-20 border-gray-600`}>
            {CURRENCIES.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
          <input type="number" value={newShares} onChange={e => setNewShares(e.target.value)}
            onKeyDown={handleKeyDown} placeholder="Shares" min="0"
            className={`${inputBase} w-20 placeholder-gray-600 border-gray-600`} />
          <div className="flex items-center gap-1">
            ...
            <input type="number" value={newCost} onChange={e => setNewCost(e.target.value)}
              onKeyDown={handleKeyDown} placeholder="PRU" min="0" step="0.01"
              className={`${inputBase} w-24 placeholder-gray-600 border-gray-600`} />
          </div>
          <button onClick={addRow} className="px-3 py-1 text-sm bg-gray-700 text-gray-300 rounded hover:bg-gray-600 transition-colors">+ Add</button>
      ...
      <p className="text-xs text-gray-600">
        Saving commits your portfolio to the GitHub repo. The cron job fetches prices every 15 min during market hours.
        Product types are auto-detected from Yahoo Finance.
      </p>
    </div>
  )
}
